class GameState {
    ...
    setCaculateCoefficientEffect(sector) {
        ...
        const givenSectorIndex = sec.indexOf(sector);
        this.sectors[sector].milestone.coefficientEffect = 0;
        for (var i = 0; i < sec.length; i++) {
            this.sectorName = sec[i];
            this.sectors[sector].milestone.coefficientEffect += this.sectors[this.sectorName].milestone.coefficientList[givenSectorIndex];
        }
    }
  ...
  */
    ...
    updateSocialMoodToAllSectors() {
        for (const sectorName in SECTORS) {
            this.sectors[sectorName].socialMood = this.socialMood;
    /**
    * updateSocialMood to all Sector because sectors need socialMood to calculate effectiveness 
    */
